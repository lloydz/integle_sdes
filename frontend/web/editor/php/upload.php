...
*/
require ("private.php");
require_once 'common_picture.php';
...
//     $GLOBALS["sPathFileName"] = $GLOBALS["sContentPath"] . $GLOBALS["sSaveFileName"];
    $encryName = getSourcePicture($GLOBALS['savePath'], $GLOBALS['sSaveFileName']);
    $GLOBALS["sPathFileName"] = $GLOBALS['uploadUrl'] . $encryName . '?savename=' . $GLOBALS['sSaveFileName'] . '&dep_path=' . $GLOBALS['savePath'];
    
    OutScript("var obj=parent.EWIN; try{obj.addUploadFile('" 
            . $GLOBALS["sOriginalFileName"] . "', '" . $GLOBALS["sPathFileName"] . "');} catch(e){} " 
            . $s_SmallImageScript . " parent.UploadSaved('" . $GLOBALS["sPathFileName"] 
            . "','" . $s_SmallImagePathFile . "', '" . $GLOBALS['sSaveFileName'] . "', '" . $GLOBALS['savePath'] . "');");
}
...
function makeImageSY($s_Path, $s_File) {
        ...
        switch ($GLOBALS["nSLTSYObject"]) {
            case 0:
                ...
                switch ($GLOBALS["nSLTCheckFlag"]) {
                    case 0:
                        if ($nOriginalWidth <= $GLOBALS["nSLTMinSize"]) {
                            return false;
                        }
                    ...
                    case 1:
                        if ($nOriginalHeight <= $GLOBALS["nSLTMinSize"]) {
                            return false;
                        }
                    ...
                    case 2:
                        if (($nOriginalWidth <= $GLOBALS["nSLTMinSize"]) && ($nOriginalHeight <= $GLOBALS["nSLTMinSize"])) {
                            return false;
                        }
    ...
    function DoRemote() {
        ...
        echo "<html><head><title>eWebEditor</title><meta http-equiv='Content-Type' content='text/html; charset=utf-8'></head><body>" . "<input type=hidden id=UploadText value=\"" . HTMLEncode($sContent) . "\">" . "</body></html>";
    ...
    function GetOriginalFileNamePre($s_FileName, $s_OkExt) {
        ...
        if (strtolower($s_Ext) != strtolower($s_OkExt)) {
            return "";
        }
    ...
    function InitUpload() {
        ...
        global $savePath, $saveName, $uploadPath, $uploadUrl;
        
        $savePath = createDeepPath();
//         $savePath = '0019/1181';
        $uploadPath = UPLOAD_PATH;//'\\\\192.168.100.18\\uploads\\graphics\\srcpath\\';
        $sUploadDir = $uploadPath . $savePath;
        mkdirs($sUploadDir);
       
        $uploadUrl = UPLOAD_URL;//'http://dev.pic.integle.com/';
        ...
        $nUploadObject = (int)$aStyleConfig[20];//0
        $sAutoDir = $aStyleConfig[71];//''

       
       /*   $sUploadDir = $aStyleConfig[3];//uoloadfile/
        if ($sBaseUrl != "3") {
        ...
        switch ($sBaseUrl) {
            case "0":
            case "3":
                $sContentPath = $sUploadDir;//$aStyleConfig[23];
                break;

            case "1":
                $sContentPath = $sUploadDir;//RelativePath2RootPath($sUploadDir);
                break;

            case "2":
                $sContentPath = $sUploadDir;//RootPath2DomainPath(RelativePath2RootPath($sUploadDir));
                break;
        }
