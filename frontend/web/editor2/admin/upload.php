...
function ShowList() {
    ...
    echo "<table border=0 cellspacing=1 align=center class=navi>" . "<form action='?' method=post name=queryform>" . "<tr><th>" . $GLOBALS["sPosition"] . "</th></tr>" . "<tr><td align=right><b>显示模式：</b>" . $s_FormViewMode . " <b>选择样式目录：</b><select name='id' size=1 onchange=\"this.form.submit()\">" . InitSelectStyle($GLOBALS["sStyleID"], "选择...") . "</select></td></tr>" . "</form></table><br>";
    if ($GLOBALS["sCurrDir"] == "") {
        return;
    }
    $s_QueryStr = "?id=" . $GLOBALS["sStyleID"] . "&d_viewmode=" . $s_ViewMode . "&dir=" . urlencode($GLOBALS["sDir"]);
    echo "<table border=0 cellspacing=1 class=list align=center>" . "<form action='" . $s_QueryStr . "&action=del' method=post name=myform>" . "<tr align=center>" . "<th width='10%'>类型</th>" . "<th width='40%'>文件地址</th>" . "<th width='10%'>大小</th>" . "<th width='15%'>最后访问</th>" . "<th width='15%'>上传日期</th>" . "<th width='10%'>删除</th>" . "</tr>";
    $nPageSize = 20;
    ...
    if ($GLOBALS["sDir"] != "") {
        echo "<tr align=center>" . "<td><img border=0 src='../sysimage/file/folderback.gif'></td>" . "<td align=left colspan=5><a href=\"?id=" . $GLOBALS["sStyleID"] . "&d_viewmode=" . $s_ViewMode . "&dir=";
...
if (isset($oDirs)) {
    foreach ($oDirs as $oDir) {
        ...
        echo "<tr align=center>" . "<td><img border=0 src='../sysimage/file/folder.gif'></td>" . "<td align=left colspan=4><a href=\"?id=" . $GLOBALS["sStyleID"] . "&d_viewmode=" . $s_ViewMode . "&dir=";
        if ($GLOBALS["sDir"] != "") {
            echo urlencode($GLOBALS["sDir"] . "/" . $oDir);
        } else {
            echo urlencode($oDir);
        }
        echo "\">" . $oDir . "</a></td>" . "<td><a href='" . $s_QueryStr . "&action=delfolder&foldername=" . urlencode($oDir) . "'>删除</a></td></tr>";
    }
}
if (isset($oFiles)) {
...
if ($nFileNum > 0) {
    ...
    foreach ($oFiles as $oFile) {
        ...
        if (($i > ($nCurrPage - 1) * $nPageSize) && ($i <= $nCurrPage * $nPageSize)) {
            ...
            if ($s_ViewMode == "list") {
                echo "<tr align=center>" . "<td>" . FileName2Pic($oFile) . "</td>" . "<td align=left><a href=\"" . $GLOBALS["sCurrPath"] . $oFile . "\" target=_blank>" . $oFile . "</a></td>" . "<td>" . filesize($sFileName) . " B </td>" . "<td>" . date("Y-m-d", fileatime($sFileName)) . "</td>" . "<td>" . date("Y-m-d", filectime($sFileName)) . "</td>" . "<td><input type=checkbox name='delfilename[]' value=\"" . $oFile . "\"></td></tr>";
            } else {
                $n = $n + 1;
                $m = $n % 4;
                if ($n == 1) {
                ...
                echo "<td width='25%' valign=top align=center><table border=0 cellspacing=1>" . "<tr><td width=200 height=200 align=center>" . File2Preview($GLOBALS["sCurrPath"], $oFile) . "</td></tr>" . "<tr><td>文件名称：<a href=\"" . $GLOBALS["sCurrPath"] . $oFile . "\" target=_blank>" . $oFile . "</a></td></tr>" . "<tr><td>文件大小：" . filesize($sFileName) . " B</td></tr>" . "<tr><td>最后访问：" . date("Y-m-d", fileatime($sFileName)) . "</td></tr>" . "<tr><td>创建日期：" . date("Y-m-d", filectime($sFileName)) . "</td></tr>" . "<tr><td>操作选择：<input type=checkbox name='delfilename[]' value=\"" . $oFile . "\"></td></tr>" . "</table></td>";
                if ($m == 0) {
                    echo "</tr>";
                }
            }
...
function File2Preview($s_Path, $s_File) {
    ...
    switch ($sExt) {
        ...
        case "SWF":
            $s_Result = "<object classid='clsid:d27cdb6e-ae6d-11cf-96b8-444553540000' codebase='http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,0,0' width=180 height=180>" . "<param name='movie' value='" . $s_PathFile . "'>" . "<param name='quality' value='high'>" . "<embed src='" . $s_PathFile . "' quality='high' width=180 height=180 type='application/x-shockwave-flash\' pluginspage='http://www.macromedia.com/go/getflashplayer'></embed>" . "</object>";
            break;
